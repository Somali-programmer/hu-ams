      ...
      {view === 'overview' && (
        <>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
            {[
              { label: 'Compliance Rate', value: '94.2%', icon: ShieldCheck, color: 'text-brand-primary', bg: 'bg-brand-primary/10' },
              { label: 'Students At Risk', value: studentsAtRisk.length, icon: AlertCircle, color: 'text-brand-primary', bg: 'bg-brand-primary/10' },
              { label: 'Total Audit Logs', value: attendance.length, icon: BarChart3, color: 'text-brand-primary', bg: 'bg-brand-primary/10' }
            ].map((stat, i) => (
              <motion.div
                key={stat.label}
              ...
              >
                <div className={cn("w-12 h-12 rounded-xl flex items-center justify-center mb-4 shadow-inner", stat.bg, stat.color)}>
                  <stat.icon className="w-6 h-6" />
                </div>
                <p className="hu-label mb-1">{stat.label}</p>
                <p className="text-2xl md:text-3xl font-serif font-bold text-brand-text">{stat.value}</p>
              </motion.div>
            ))}
          </div>
          ...
          {/* Quick Actions Grid */}
          <section className="grid grid-cols-1 md:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6">
            {[
              { title: 'Audit Logs', desc: 'Review system compliance and attendance logs.', icon: ShieldCheck, color: 'text-brand-primary', bg: 'bg-brand-primary/10', path: '/qa/audit' },
              { title: 'Corrections', desc: 'Review and approve attendance correction requests.', icon: ClipboardCheck, color: 'text-brand-primary', bg: 'bg-brand-primary/10', path: '/qa/corrections' },
              { title: 'Reports', desc: 'Generate verification and compliance reports.', icon: BarChart3, color: 'text-brand-primary', bg: 'bg-brand-primary/10', path: '/qa/reports' }
            ].map((action) => (
        ...
        <>
          <section className="space-y-6 md:space-y-8">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              <h2 className="text-2xl md:text-3xl font-serif font-bold text-brand-text">Attendance Compliance Audit</h2>
              <div className="flex items-center gap-4 w-full md:w-auto">
                <div className="relative flex-1 md:w-64">
                  ...
                  <input
                    ...
                    className="w-full pl-12 pr-6 py-3 bg-hu-cream/30 border-none rounded-xl text-xs font-bold focus:ring-2 focus:ring-brand-primary/20 outline-none transition-all"
                  />
                </div>
              </div>
            </div>
                ...
                <table className="w-full text-left min-w-[800px]">
                  <thead>
                    <tr className="bg-brand-surface">
                      ...
                      <th className="px-4 py-4 md:px-8 md:py-6 text-[11px] uppercase tracking-[0.2em] font-bold text-brand-muted whitespace-nowrap">Instructor</th>
                      <th className="px-8 py-6 text-[11px] uppercase tracking-[0.2em] font-bold text-brand-muted whitespace-nowrap">Avg. Attendance</th>
                      <th className="px-8 py-6 text-[11px] uppercase tracking-[0.2em] font-bold text-brand-muted whitespace-nowrap">Status</th>
                      <th className="px-8 py-6 text-[11px] uppercase tracking-[0.2em] font-bold text-brand-muted whitespace-nowrap">Last Audit</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-brand-border">
                    <tr className="hover:bg-hu-cream/10 transition-colors group">
                      ...
                      <td className="px-8 py-6 text-sm font-bold text-brand-text whitespace-nowrap">92%</td>
                      <td className="px-8 py-6 whitespace-nowrap">
                        <span className="px-4 py-1.5 bg-brand-primary/10 text-brand-primary rounded-full text-[10px] font-bold uppercase tracking-widest border border-brand-primary/20">
                          Compliant
                        </span>
                      </td>
                    ...
                    <tr className="hover:bg-hu-cream/10 transition-colors group">
                      <td className="px-8 py-6 whitespace-nowrap">
                        <div className="flex flex-col">
                          <span className="text-sm font-bold text-brand-text">Web Development</span>
          ...
          <section className="space-y-4">
            <h2 className="text-xl font-bold text-brand-text text-red-600 flex items-center gap-2">
              ...
              {studentsAtRisk.map(student => (
                <div key={student.userId} className="bg-white p-6 rounded-3xl shadow-sm border border-red-100 space-y-3">
                  <div className="flex justify-between items-start">
                    <div>
                      <h3 className="font-bold text-brand-text">{student.fullName}</h3>
      ...
      {view === 'corrections' && (
        <section className="space-y-8">
          <div className="flex items-center gap-4">
            ...
            <h2 className="text-2xl md:text-3xl font-serif font-bold text-brand-text">Attendance Correction Requests</h2>
          </div>
          <div className="hu-card-alt overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full text-left min-w-[800px]">
                <thead>
                  <tr className="bg-brand-surface">
                    <th className="px-4 py-4 md:px-8 md:py-6 text-[11px] uppercase tracking-[0.2em] font-bold text-brand-muted whitespace-nowrap">Student</th>
                  <th className="px-8 py-6 text-[11px] uppercase tracking-[0.2em] font-bold text-brand-muted whitespace-nowrap">Course & Date</th>
                  <th className="px-8 py-6 text-[11px] uppercase tracking-[0.2em] font-bold text-brand-muted whitespace-nowrap">Reason</th>
              ...
              <tbody className="divide-y divide-brand-border">
                {[
                  ...
                  <tr key={req.id} className="hover:bg-hu-cream/10 transition-colors">
                    <td className="px-8 py-6 whitespace-nowrap">
                      <p className="text-sm font-bold text-brand-text">{req.name}</p>
                    </td>
                    <td className="px-8 py-6 whitespace-nowrap">
                      <p className="text-sm font-medium text-brand-text">{req.course}</p>
      ...
      {view === 'reports' && (
        <section className="space-y-8">
          <div className="flex items-center gap-4">
            ...
            <h2 className="text-2xl md:text-3xl font-serif font-bold text-brand-text">Verification Reports</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 xl:grid-cols-4 gap-8">
            {[
              ...
              <div key={report.title} className="hu-card-alt p-5 md:p-4 md:p-6 space-y-6">
                <div className="w-10 h-10 md:w-12 md:h-12 bg-hu-cream rounded-xl flex items-center justify-center text-brand-primary">
